...
const columns = ['Status', 'Course', 'Name', 'Dates', 'Task', 'Notes'] as const;
...
export default function SchoolTasksTable({ tasks }: Props) {
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden">
      <div className="grid grid-cols-[80px_140px_1.3fr_180px_140px_1fr] bg-muted text-xs font-medium text-muted-foreground">
        {columns.map((c) => (
      ...
      <div className="divide-y divide-border">
        {sorted.map((t) => {
          ...
          return (
            <div
              key={t.id}
              className="grid grid-cols-[80px_140px_1.3fr_180px_140px_1fr] text-sm hover:bg-muted/60"
            >
